...
The value of these laser pulse timings, and the time-of-flight value from the diffuser ball are subtracted from the PMT hit time. Using the observed charge values and these adjusted hit times, "TQ" (time and charge) distributions can be plotted for each inner detetector PMT. An example TQ distribution is shown for an inner detector PMT (cable number 00010) in Figure \ref{fig:TQdist}, where the vertical axis shows is the TOF corrected and laser pulse time corrected PMT hit time, the horizontal axis shows the observed charge of each hit. Figure taken from \cite{abeCalibrationSuperKamiokandeDetector2014}. 
...
\begin{table}[H]
    $$
    \begin{array}{ll}
    \text {Material} & \text { Refractive index } \\
    Water & 1.33 \\
    Glass & 1.472+3670/\lambda^{2} \\
    Bi-alkali & n_{real} + i \dot n_{img} \\
    Vacuum & 1.00 \\
    x_{a b s}^{N} & \text { Nucleon absorption probability }  \\
    x_{\pi}^{N} & \text { Nucleon } \pi \text {-production probability }
    \end{array}
    $$
...
Using the nickel source mentioned in Section \refsection{Absolute gain calibration}, the dependence between the amount of light attenuation and vertical position in the tank is calculated, with the equation for this model shown in \ref{eq:TBAeq}, where $<N_{top}>$, $<N_{bottom}>$ and $<N_{barrel}>$ are the mean rate at which the PMTs at the top, bottom and barrel regions are hit.  
