...
\begin{equation}
    n_{VTX}=\underset{r}{\operatorname{argmin}}\left(\sqrt{\frac{1}{N 10_{p v x}} \sum_{i=1}^{N 10_{p v x}}\left(\tau_i(\boldsymbol{r})-\bar{\tau}(\boldsymbol{r})\right)^2}\right)
\label{eq:ncap_vtx}
\end{equation}
...
his is the number of hits within -100 ns and +100 ns from the fitted time relative to the trigger.