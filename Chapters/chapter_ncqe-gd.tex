...
Table \ref{table:ambe_tau_chi2} shows the value of tau for each of these positions, along with the $\chi^{2}$/ndf value for the exponential fit and the value of the neutron capture efficiency calculated from the Am/Be data at each of these positions. Equation \ref{eq:efficiency_ambe_calculation} shows the method by which the efficiency for the neutron capture on Am/Be data is calculated, where $B$ is the fitted constant background value, and $NTag TMAX$ and $NTag TMIN$ are the end and start times of the neutron tagging algorithm.  



\begin{equation}
    \text { Efficiency }=\frac{(\text {# of neutron captures } - (\text{B} \cdot \text {# of bins }  \cdot (\text{NTag TMAX - NTag TMIN}) / (\text{Exponential fit time range})))  }{\# \text { Number of events }}
\end{equation}
